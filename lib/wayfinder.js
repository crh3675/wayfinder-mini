...
Wayfinder.prototype.setStartingCoords = function(start_x, start_y, callback) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let board_width = self.matrix[0].length;
        ...
        // Subtract board height -1 as our 0,0 is bottom left
        self.start_x = start_x;
        self.start_y = board_height - start_y - 1;
...
Wayfinder.prototype.__move = function(direction) {
    ...
    if(cell && cell !== 0) {
        ...
        let response = { final_x :  self.current_x, final_y : (board_height - self.current_y - 1), coins : self.coins, moves  : self.moves };  
        
        if(self.coins === self.movements.length) {
            console.log('Notice: Level completed!');      
        } else {
