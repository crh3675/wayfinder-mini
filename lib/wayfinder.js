 ...
 */
Wayfinder.prototype.init = function(board, movements, random, callback) {
    
    const self = this;
    
    return new Promise((resolve, reject) => {
       
       self.loadBoard(board, random, callback)
       .then(config => {
 ...
 */
Wayfinder.prototype.loadBoard = function(board, random, callback) {
    const self = this;
...
Wayfinder.prototype.buildRandomBoard = function(random, callback) {
    ...
    let wall_factor = Math.floor(Math.random() * 10 + 1);
        ...
        if(random.match(/(\d+)x(\d+)/)) {
            ...
            for(let y = 0; y < bound_y; y++) {
                for(let x = 0; x < bound_x; x++) {
                    ...
                    config.matrix[y][x] = parseInt(Math.random() * 100, 10) % wall_factor ? 1 : 0;
                }    
            }
